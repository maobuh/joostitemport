...
namespace joostitemport.Items
{
    public class DukeFishingGun : ModItem
    {
        ...
        public override void AddRecipes()
        {
            ...
            recipe2.AddIngredient(null, "GoldenFishingGun", 1);
            recipe2.Register();

            recipe3.AddIngredient(null, "MechanicalFishingGun", 1);
            recipe3.Register();

            recipe4.AddIngredient(null, "FishingGun", 1);
            recipe4.Register();

            recipe5.AddIngredient(ItemID.Boomstick, 1);
            recipe5.Register();
        }
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            const float spread = 65f * 0.0174f;
            float baseSpeed = velocity.Length();
            float startAngle = (float) Math.Atan2(velocity.X, velocity.Y) - (spread / 2);
            const float deltaAngle = spread / 16f;
            for (int i = 0; i < 16; i++)
            {
